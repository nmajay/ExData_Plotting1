...
SummedUpEmissions <- with(NEI, aggregate(NEI[,'Emissions'], by = list(year),sum,na.rm=TRUE))
names(SummedUpEmissions) <- c('Year','TotalEmissions')
#head(SummedUpEmissions)

#Setting up the Printing device to store the graph
png(filename='ExData_Plotting1/Project2/plot1.png', width=480, height=480, units='px')
plot(SummedUpEmissions,type = "l", xlab = "Year", ylab = "Total PM2.5 Emissions"
        ,xaxt = "n", main = "Total PM2.5 Emission Trend Across Years", col = "blue")
